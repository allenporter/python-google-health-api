...
async def main() -> None:
    ...
    try:
        ...
        print(
            "Please install the dev dependencies first: uv pip install -r requirements_dev.txt"
        )
        sys.exit(1)
    ...
    print(
        f"Access Token (preview): {credentials.token[:15]}...{credentials.token[-15:]}"
    )
    ...
    async with aiohttp.ClientSession() as session:
        ...
        try:
            result = await api.steps.list(
                start_time=start_time, end_time=end_time, page_size=5
            )
        ...
        try:
            hr_result = await api.heart_rate.list(
                start_time=start_time, end_time=end_time, page_size=5
            )
