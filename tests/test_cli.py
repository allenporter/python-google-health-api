...
def test_cli_profile_commands(mock_load, mock_setup, capsys) -> None:
    ...
    with patch.object(sys, "argv", ["google-health-cli", "profile", "update"]):
        with pytest.raises(SystemExit):
            main()
    captured = capsys.readouterr()
...
def test_cli_settings_commands(mock_load, mock_setup, capsys) -> None:
    ...
    with patch.object(sys, "argv", ["google-health-cli", "settings", "update"]):
        with pytest.raises(SystemExit):
            main()
...
def test_cli_subscribers_commands(mock_load, mock_setup, capsys) -> None:
    ...
    with patch.object(sys, "argv", ["google-health-cli", "subscribers", "create"]):
        with pytest.raises(SystemExit):
            main()
    ...
    # subscribers delete
    mock_sub_api.delete.return_value = MagicMock(spec=["to_dict"], to_dict=lambda: {})
    with patch.object(
        sys,
        "argv",
        [
...
def test_cli_datatype_crud_and_options(mock_load, mock_setup, capsys) -> None:
    ...
    with patch.object(sys, "argv", ["google-health-cli", "steps", "rollup"]):
        with pytest.raises(SystemExit):
            main()
    captured = capsys.readouterr()
